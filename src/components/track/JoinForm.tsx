...
export function JoinForm({ queue, onJoin }: JoinFormProps) {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    if (!success) {
      toast.error(error || t("join.failedToJoin"));
      setLoading(false);
      return;
    }
    ...
    if (data && data.ticket_id) {
      if (whatsapp && whatsapp.sentViaService) {
        toast.success(t("join.ticketIssuedSent", { phone }));
      } else {
        toast.success(t("join.ticketIssuedQueued", { phone }));
      }
      ...
      if (isIOS && !isStandalone) {
        toast(t("join.iosAddToHome"), { icon: "📱", duration: 8000 });
        ...
        try {
          ...
          if (permission === 'granted') {
            ...
            toast.success(t("join.notificationsEnabled"));
          } else {
            toast(t("join.enableNotifications"), { icon: "🔔" });
          }
        } catch (err) {
      ...
      toast.error(t("join.invalidResponse"));
    }
    setLoading(false);
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-surface border border-border rounded-3xl p-8 md:p-12 premium-shadow">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold font-sans text-foreground mb-3 tracking-tight">{t("join.title")}</h2>
        <div className="bg-muted p-3 rounded-xl mb-4 text-left border border-border">
          ...
          <div className="mt-1">
            <DoctorRatingBadge queueId={queue.id} />
          </div>
        </div>
