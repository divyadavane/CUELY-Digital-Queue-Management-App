    ...
    <section id="how-it-works" className="py-24 relative overflow-hidden bg-[var(--bg-secondary)] border-y border-[var(--border-color)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-1/2 left-12 right-12 h-px border-t-2 border-dashed border-[var(--border-color)] -translate-y-1/2 z-0" />

          <RevealOnScroll stagger className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8 relative z-10">
            {HOW_IT_WORKS_STEPS.map((step, idx) => (
              <GlowCard key={step.number} className="h-full flex flex-col justify-between relative bg-[var(--bg-card)] backdrop-blur-xl" glowColor="rgba(59, 130, 246, 0.2)">
                <div>
                  {/* Step Number Badge */}
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] font-mono flex items-center gap-1">
                  ...
                  <ul className="space-y-2.5 pt-4 border-t border-[var(--border-color)]">
                    {step.details.map((detail, dIdx) => (
                      <li key={dIdx} className="flex items-center gap-2 text-[11px] text-[var(--text-secondary)] font-medium">
